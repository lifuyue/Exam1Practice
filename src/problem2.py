...
def problem2a(circle, rectangle, window):
    ...
    window.render()
    window.continue_on_mouse_click('To continue, click anywhere in this window')
...
def problem2b(rect, n, delta, win):
    ...
    # ------------------------------------------------------------------
    rect.attach_to(win)
    x1 = rect.get_upper_left_corner().x - delta
    y1 = rect.get_upper_left_corner().y - delta
    x2 = rect.get_lower_right_corner().x + delta
    y2 = rect.get_lower_right_corner().y + delta
    for _ in range(n - 1):
